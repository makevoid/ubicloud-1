  ...
  context "when Stripe enabled" do
    ...
    describe "discount code" do
      ...
      it "can apply a valid discount code" do
        DiscountCode.create_with_id(discount_code: "VALID_CODE", credit_amount: 33, expires_at: Time.now + 86400)
        visit "#{project.path}/billing"
      ...
      it "shows error for expired discount code" do
        DiscountCode.create_with_id(discount_code: "EXPIRED_CODE", credit_amount: 33, expires_at: Time.now - 86400)
        visit "#{project.path}/billing"
        fill_in "Discount Code", with: "EXPIRED_CODE"
        click_button "Apply"

        expect(page).to have_flash_error "Discount code has expired."
        expect(page).to have_content "$0.00"
        expect(project.reload.credit).to eq(0.00)
      end

      it "shows error if discount code has already been applied" do
        used_discount_code = DiscountCode.create_with_id(discount_code: "USED_CODE", credit_amount: 33, expires_at: Time.now + 86400)
